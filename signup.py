...
def main():
    ...
    if name != '' and lastname != '' and email != '' and username != '' and password != '' and re_password != '':
        ...
        if len(username) <= 4:
            messagebox.showerror('Error', 'Username must be longer than 4 characters')
        # Check if the username already exists in the database
        elif cursor.execute('SELECT username FROM users WHERE username=?', (username,)).fetchone() is not None:
